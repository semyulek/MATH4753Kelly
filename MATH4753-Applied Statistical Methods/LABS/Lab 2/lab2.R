...
#}
getwd()
mpg.df=readxl::read_xls("MATH4753-Applied Statistical Methods/LABS/Lab 2/EPAGAS.XLS")
mpg.df
...
## What is actually the case?
length(q[abs(z)<2])/length(q)

## Can you see that this is the same as
length(q[mean(q)-2*sd(q) < q & q < mean(q)+ 2*sd(q) ])/length(q)
